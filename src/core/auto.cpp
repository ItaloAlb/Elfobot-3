...
void Auto::Fishing(bool& controller) {
	...
	while (controller) {
		...
		if (fishing_location.IsValid() && *POINTER::CLIENT_STATE == 1) {
			...
			//std::this_thread::sleep_for(milliseconds(controller_fishing_cooldown * 1000));
		}	
	}
}
...
Ability* Auto::SelectBestAbility(const std::vector<Ability>& moveset, int situation) {
	...
	for (const auto& ability : moveset) {
		...
		if (score > bestScore) {
			bestScore = score;
			// Remove const by taking the address from a non-const container if needed.
